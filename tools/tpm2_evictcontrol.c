...
static tpm_evictcontrol_ctx ctx = {
    .auth_hierarchy.object.tr_handle = ESYS_TR_RH_OWNER,
};
...
static bool on_option(char key, char *value) {
    ...
    case 'a':
        result = tpm2_hierarchy_from_optarg(value,
            ...
            TPM2_HIERARCHY_FLAGS_ESAPI|TPM2_HIERARCHY_FLAGS_O|TPM2_HIERARCHY_FLAGS_P);
        if (!result) {
            return false;
        }
        break;
...
tool_rc tpm2_tool_onrun(ESYS_CONTEXT *ectx, tpm2_option_flags flags) {
    ...
    rc = tpm2_util_object_load_auth(ectx, NULL, ctx.auth_hierarchy.auth_str,
        &ctx.auth_hierarchy.object, false);
