...
tool_rc nv_readlock(ESYS_CONTEXT *ectx) {
...
static tool_rc process_inputs(ESYS_CONTEXT *ectx) {
    ...
    tpm2_handle_flags valid_handles =
        TPM2_HANDLE_FLAGS_O | TPM2_HANDLE_FLAGS_P | TPM2_HANDLE_FLAGS_NV;
    if (ctx.is_tcti_none) {
...
static bool tpm2_tool_onstart(tpm2_options **opts) {
    ...
    *opts = tpm2_options_new("C:P:S:n:", ARRAY_LEN(topts), topts, on_option,
        on_arg, TPM2_OPTIONS_FAKE_TCTI|TPM2_OPTIONS_NO_OUTPUT);
...
TPM2_TOOL_REGISTER("nvreadlock", tpm2_tool_onstart, tpm2_tool_onrun,
