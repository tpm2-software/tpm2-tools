...
cleanup() {
    ...
    do
        handle=$(printf "0x%X\n" $(($handle_base + $idx)))
        tpm2_evictcontrol -Q -C "$auth" -c "$handle"
    done
...
for idx in "${!keys[@]}"
do
    tpm2_createprimary -Q -C "$auth" -g "${hashes[$idx]}" -G "${keys[$idx]}" -o primary.context
    handle=$(printf "0x%X\n" $(($handle_base + $idx)))
    tpm2_evictcontrol -Q -C "$auth" -p "$handle" -c primary.context
done
